...
const Tool = ({
  ...
  const desc = typeof description === 'function' ? description(t) : description;
  const [isHovered, setIsHovered] = useState(false);

  useEffect(() => {
    if (selectedTool !== desc) {
      setIsHovered(false);
    }
  }, [selectedTool, desc]);
      ...
      <TooltipTrigger asChild>
        <motion.div
          className={cx('p-3 rounded-full', {
            'bg-primary !text-primary-foreground': selectedTool === desc,
          })}
          onHoverStart={() => {
            setIsHovered(true);
          }}
          onHoverEnd={() => {
            if (selectedTool !== desc) setIsHovered(false);
          }}
          onKeyDown={(event) => {
            if (event.key === 'Enter') {
              handleSelect();
        ...
        >
          {selectedTool === desc ? <ArrowUpIcon /> : icon}
        </motion.div>
      </TooltipTrigger>
      <TooltipContent
        side="left"
      ...
      <AnimatePresence>
        {isToolbarVisible &&
          secondaryTools.map((secondaryTool) => (
            <Tool
              key={
                typeof secondaryTool.description === 'function'
                  ? secondaryTool.description(t)
                  : secondaryTool.description
              }
